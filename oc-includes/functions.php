...
{
        $config_file = 'oc-config/config.ini';
...
function bad_word($term){
        $banned_words = @file_get_contents(  $_SERVER['DOCUMENT_ROOT'] .'/oc-includes/class/badkeyword.txt' );
...
function bad_words($content){
        $banned_words = @file_get_contents(  DOCUMENT_ROOT .'/oc-includes/class/badkeyword.txt' );
...
// Hak Cipta OcimScripts Di Lindungi Oleh Tuhan Yang Maha Esa //
// Jika anda menyebarkan Scripts ini secara ilegal, semoga rezeki anda terhambat //
...
function save_word($query){
	...
	$f     = 0;
        $koma  = ",";
        $root  = DOCUMENT_ROOT .'/oc-content/keywords/'; 
