...
function site_theme() {
    return site_url().'/oc-content/themes/'.config('sitethemes');
}
function style_theme() {
    echo site_url().'/oc-content/themes/'.config('sitethemes');
}
function get_header( ) {
        if (file_exists( $_SERVER['DOCUMENT_ROOT'] . '/oc-content/themes/'. config('sitethemes') . '/header.php' ))
                include ( $_SERVER['DOCUMENT_ROOT'] . '/oc-content/themes/' . config('sitethemes') . '/header.php' );
}
function get_footer( ) {
        if (file_exists( $_SERVER['DOCUMENT_ROOT'] . '/oc-content/themes/'. config('sitethemes') . '/footer.php' ))
                include ( $_SERVER['DOCUMENT_ROOT'] . '/oc-content/themes/' . config('sitethemes') . '/footer.php' );
}
function get_sidebar( ) {
        if (file_exists( $_SERVER['DOCUMENT_ROOT'] . '/oc-content/themes/'. config('sitethemes') . '/sidebar.php' ))
                include ( $_SERVER['DOCUMENT_ROOT'] . '/oc-content/themes/' . config('sitethemes') . '/sidebar.php' );
}
function _seo( $query ) {
        if ( config('_seo') == 'true' ):
...
function oc_title( $separate = ' | ', $options = array() ){
	...
	$options = array_merge($defaults, $options);
        ...
        elseif ( isset($_GET['do']) || strpos(dirname($uri), options('url_page') ) !== false ) {
                ...
                if ( $options['before'] == '' ) : $before = 'Pages '; else: $before = $options['before']; endif;
                if ( $options['after'] == '' ) : $after = ''; else: $after = $options['after']; endif;
                $oc_title = $before . $deskripsi . $after. $separate . $sep;
        } 
...
function tluo920(){
        $our = '<div class="container" style="margin: 30px;background-color: #fff;"><table class="table" style="width: 700px;max-width: 700px;margin: 0 auto;padding: 20px 0;"><tr><td style="vertical-align:middle;width:30%">License Key Required!</td><td style="width:70%">This domain is not registered for this license. You can get this from <a href="http://member.ocimscripts.com/" target="_blank">OcimScripts.Com</a></td></tr></table></div>';
        echo $our;
        exit();
}
...
function recent_search($limit = 10, $pos = 0) {
	$word_path = DOCUMENT_ROOT .'/oc-content/keywords/'; 
